      ...
      <el-table-column label="操作" align="center" class-name="small-padding fixed-width">
        <template slot-scope="scope">
          ...
          <el-button
            ...
            v-hasPermi="['system:player:remove']"
          >删除</el-button>
        </template>
      </el-table-column>
    </el-table>
  ...
  methods: {
    ...
    getList() {
      ...
      listPlayer(this.queryParams).then(response => {
        this.playerList = this.handleTree(response.data, "username", "id");
        this.loading = false;
      });
    },
    /** 转换玩家管理3数据结构 */
    ...
    getTreeselect() {
      listPlayer().then(response => {
        this.playerOptions = [];
        const data = { username: 0, level: '顶级节点', children: [] };
        data.children = this.handleTree(response.data, "username", "id");
        this.playerOptions.push(data);
      });
    },
    // 取消按钮
    cancel() {
    ...
    handleAdd(row) {
      this.reset();
      this.getTreeselect();
      if (row != null && row.username) {
        this.form.id = row.username;
      } else {
        this.form.id = 0;
      }
      this.open = true;
    ...
    handleUpdate(row) {
      ...
      getPlayer(row.id).then(response => {
    ...
    /** 删除按钮操作 */
    handleDelete(row) {
      this.$modal.confirm('是否确认删除玩家管理3编号为"' + row.id + '"的数据项？').then(function() {
