...
<script>
import { getAuditHistory,auditCancel,getAuditProcess,listCarPackageAudit, getCarPackageAudit, delCarPackageAudit, addCarPackageAudit, updateCarPackageAudit } from "@/api/business/carPackageAudit";
...
export default {
  ...
  methods: {
    ...
    getList() {
      ...
      listCarPackageAudit(this.queryParams).then(response => {
    ...
    /** 撤销按钮操作 */
    handleCancel(id) {
      this.$modal.confirm('是否确认撤销流程定义明细编号为"' + id + '"的数据项？').then(function() {
        return auditCancel(id);
      }).then(() => {
        this.getList();
        this.$modal.msgSuccess("撤销成功成功");
      }).catch(() => {});
    },
